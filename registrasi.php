...
if (isset($_POST['register'])) {
    ...
    if (!empty($nama) && !empty($email) && !empty($password)) {
        ...
        if ($hasil_cek->num_rows > 0) {
            ...
            // --- FITUR HASHING DITAMBAHKAN DI SINI ---
            // Mengacak password menggunakan algoritma default yang paling aman saat ini (Bcrypt)
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);

            // Masukkan variabel $hashed_password ke dalam query
            $query = "INSERT INTO users (email, password, nama, role, saldo) 
                      VALUES ('$email', '$hashed_password', '$nama', '$role', '$saldo')";
