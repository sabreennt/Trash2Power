...
// 3. Ambil Data User
$stmt = $conn->prepare("SELECT nama, saldo, gender, foto_profil FROM users WHERE id_user = ?");
...
// 4. Statistik Sampah (Menggunakan prepared statement agar lebih aman)
$q_stats = $conn->prepare("SELECT id_kategori, SUM(jumlah_sampah) as total FROM setoran WHERE id_warga = ? GROUP BY id_kategori");
$q_stats->bind_param("i", $id_user);
$q_stats->execute();
$res_stats = $q_stats->get_result();

while ($stat = $res_stats->fetch_assoc()) {
    if ($stat['id_kategori'] == 1) $total_botol = $stat['total'];
    if ($stat['id_kategori'] == 2) $total_kaleng = $stat['total'];
}

// 5. Logika Foto Profil
...
<body class="min-h-screen pb-32 md:pb-0">
    ...
    <main class="max-w-6xl mx-auto px-6 py-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <a href="reward.php" class="stat-card bg-[#E5E7EB] border-none shadow-sm cursor-pointer transition group hover:bg-[#D1D5DB] no-underline">
                <div class="w-16 h-16 flex items-center justify-center">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                    // Cari semua status di tabel
                    const badges = document.querySelectorAll('.badge-status');

                    badges.forEach(badge => {
                        // Jika statusnya sedang 'diproses'
                        if (badge.getAttribute('data-status') === 'diproses') {
                            const idSetoran = badge.getAttribute('data-id');

                            // Buat jeda selama 3 detik (3000 milidetik)
                            setTimeout(() => {
                                // 1. Ubah tampilan di layar jadi hijau
                                badge.className = 'badge-status bg-green-100 text-[#34A853] px-3 py-1 rounded-full text-[10px] font-black uppercase';
                                badge.innerText = 'SALDO MASUK';

                                // 2. Kirim sinyal ke database secara diam-diam (AJAX) untuk mengubah status permanen
                                // (Kamu perlu buat file update_status.php untuk menangkap sinyal ini)
                                fetch('update_status.php?id=' + idSetoran + '&status=saldo_masuk');

                            }, 3000); // Ganti 3000 menjadi 5000 jika ingin 5 detik
                        }
                    });
})
    </script>
