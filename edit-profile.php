...
// Ambil data user terbaru untuk mendapatkan nama foto lama
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($foto_ext, $allowed_ext)) {
            ...
            if (!move_uploaded_file($foto_temp, "uploads/" . $foto_new_name)) {
                ...
                $foto_new_name = $user['foto_profil'];
            }
        } else {
    ...
    // Logika bypass (true) untuk testing atau validasi normal
    if (!empty($current_password)) {
        // Ganti 'true ||' jika ingin mengaktifkan verifikasi password asli kembali nanti
        if (true || password_verify($current_password, $user['password'])) {
    ...
    if (empty($error_msg)) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            // Memunculkan pesan error spesifik jika database menolak
            $error_msg = "Gagal update database: " . $conn->error;
        }
    }
}
?>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="flex flex-col items-center justify-center text-center mb-8">
                    ...
                    <div class="flex items-center justify-center gap-3 w-full">
                        ...
                        <span id="file-name" class="text-[11px] text-gray-500 truncate max-w-[150px]">
                            <?= !empty($user['foto_profil']) ? $user['foto_profil'] : 'No File Chosen' ?>
                        </span>
                    </div>
                </div>
