...
// --- BAGIAN 1: PROSES DATA (AJAX POST) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Bersihkan output buffer untuk mencegah karakter tak diinginkan masuk ke JSON
    ob_clean();
    header('Content-Type: application/json');

    $id_user = $_SESSION['id_user'];
    $id_kategori = isset($_POST['id_kategori']) ? mysqli_real_escape_string($koneksi, $_POST['id_kategori']) : null;

    if (!$id_kategori) {
        echo json_encode(['status' => 'error', 'message' => 'ID Kategori kosong']);
        exit;
    }

    $resKategori = mysqli_query($koneksi, "SELECT jenis_sampah, harga_sampah FROM kategori_sampah WHERE id_kategori = '$id_kategori'");

    if ($resKategori && mysqli_num_rows($resKategori) > 0) {
        $dataKategori = mysqli_fetch_assoc($resKategori);
        $harga = $dataKategori['harga_sampah'];
        $nama_sampah = $dataKategori['jenis_sampah'];

        $insert = mysqli_query($koneksi, "INSERT INTO setoran (id_warga, id_kategori, jumlah_sampah, hasil_pendapatan, tgl_penyetoran, status) 
                                VALUES ('$id_user', '$id_kategori', 1, '$harga', NOW(), 'diproses')");
        ...
        if ($insert) {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Gagal simpan ke database: ' . mysqli_error($koneksi)]);
        }
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Kategori tidak ditemukan di database']);
    }
    exit;
}

// Ambil data user untuk tampilan Saldo
...
<body class="min-h-screen pb-32 md:pb-0">
    ...
    <main class="max-w-6xl mx-auto mt-10 px-4">
        ...
        <div class="mt-12 bg-white rounded-[32px] border card-shadow overflow-hidden">
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        while ($r = mysqli_fetch_assoc($q)):
                            $status_text = !empty($r['status']) ? $r['status'] : 'sedang diproses';
                            // Logika Warna
                            $badge_color = "bg-yellow-100 text-yellow-700";
                            if ($status_text == 'saldo berhasil masuk') $badge_color = "bg-green-100 text-[#34A853]";
                            if ($status_text == 'dibatalkan') $badge_color = "bg-red-100 text-red-600";
                        ?>
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="px-8 py-5">
                                    <span class="bg-blue-50 text-blue-500 px-3 py-1 rounded-full text-[10px] font-black uppercase">
                                        <?= $r['jenis_sampah'] ?>
                                    </span>
                                </td>
                                <td class="px-8 py-5 font-bold text-gray-800">Rp <?= number_format($r['hasil_pendapatan'], 0, ',', '.') ?></td>
                                <td class="px-8 py-5">
                                    <span class="<?= $badge_color ?> px-3 py-1 rounded-full text-[10px] font-black uppercase inline-block">
                                        <?= $status_text ?>
                                    </span>
                                </td>
                            </tr>
            ...
            <div class="md:hidden divide-y divide-gray-100">
                <?php
                mysqli_data_seek($q, 0); // Reset pointer query
                while ($r = mysqli_fetch_assoc($q)):
                    $status_text = !empty($r['status']) ? $r['status'] : 'sedang diproses';
                    $text_color = "text-yellow-600";
                    if ($status_text == 'saldo berhasil masuk') $text_color = "text-[#34A853]";
                    if ($status_text == 'dibatalkan') $text_color = "text-red-500";
                ?>
                    <div class="p-5 flex justify-between items-center hover:bg-gray-50 transition">
                        <div class="flex flex-col gap-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-[9px] font-black uppercase"><?= $r['jenis_sampah'] ?></span>
                            </div>
                            <span class="text-[11px] text-gray-500"><?= date('d M Y, H:i', strtotime($r['tgl_penyetoran'])) ?></span>
                            <span class="text-[10px] font-bold uppercase <?= $text_color ?>"><?= $status_text ?></span>
                        </div>
    ...
    <footer class="py-10 text-center text-gray-400 text-sm mb-24 md:mb-8">
