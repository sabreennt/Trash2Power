...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($dataKategori) {
        ...
        // Cari bagian ini di scan.php kamu
        mysqli_query($koneksi, "INSERT INTO setoran (id_warga, id_kategori, jumlah_sampah, hasil_pendapatan, tgl_penyetoran, status) 
                        VALUES ('$id_user', '$id_kategori', 1, '$harga', NOW(), 'sedang diproses')");
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        while ($r = mysqli_fetch_assoc($q)):
                            ...
                            if ($status_text == 'diproses') {
                                $badge_color = "bg-yellow-100 text-yellow-700";
                            } elseif ($status_text == 'saldo masuk') {
                                $badge_color = "bg-green-100 text-[#34A853]";
                            } else {
                        ...
                        ?>
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="px-8 py-5"><span class="bg-blue-50 text-blue-500 px-3 py-1 rounded-full text-[10px] font-black uppercase"><?= $r['jenis_sampah'] ?></span></td>
                                <td class="px-8 py-5 font-bold text-gray-800">Rp <?= number_format($r['hasil_pendapatan'], 0, ',', '.') ?></td>

                                <td class="px-8 py-5">
                                    <span class="badge-status <?= $badge_color ?> px-3 py-1 rounded-full text-[10px] font-black uppercase" data-id="<?= $r['id_setoran'] ?>" data-status="<?= $status_text ?>">
                                        <?= $status_text ?>
                                    </span>
                                </td>
                            </tr>
